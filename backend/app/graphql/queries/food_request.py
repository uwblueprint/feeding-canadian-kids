...
class FoodRequestQueries(QueryList):
    ...
    def resolve_food_requests(self, info, limit, offset, status=None):
        ...
        near_location = {
            "type": "Point",
            "coordinates": [43.6544, 79.3807]
        } 
        return services["food_request_service"].get_food_requests(limit=limit, offset=offset, status=status, near_location=near_location)
    ...
    def resolve_food_requests_by_user(self, info, limit, offset, user_id, status=None):
        ...
        return services["food_request_service"].get_food_requests_by_user(limit=limit, offset=offset, user_id=user_id, role=user_role, status=status)
