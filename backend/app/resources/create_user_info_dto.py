...
class CreateUserInfoDTO:
    ...
    def validate(self):
        ...
        if type(self.contact_email) is not str or not re.search(emailRegex, self.contact_email):
            error_list.append("The contact email supplied is not a string.")
        if type(self.contact_phone) is not str or not re.search(phoneRegex, self.contact_phone):
