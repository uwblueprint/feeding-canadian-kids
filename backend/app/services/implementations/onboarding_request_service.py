...
class OnboardingRequestService(IOnboardingRequestService):
    ...
    def reject_onboarding_request(self, request_id):
        try:
            ...
            referenced_onboarding_request.status = "Rejected" # reject the onboarding request
            ...
            reason = getattr(e, "message", None)
            self.logger.error("Failed to reject onboarding request. Reason = {reason}".format(reason=(reason if reason else str(e))))
            raise e
